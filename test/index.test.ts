import { describe, it, expect } from 'vitest';
import fs from 'node:fs'
import { toSVG } from '../src'

const logoUrl = 'data:image/jpeg;base64,'

describe('Simple QR', () => {
  it('normal string should be ok', async () => {
    const svg = toSVG("Hello, world!")
    fs.writeFileSync(__dirname + '/output/test-1.svg', svg)
    // const canvas = await svg2canvas(svg)
    // fs.writeFileSync(__dirname + '/output/test-1.jpg', canvas.toBuffer())
    expect(svg).toBeDefined()
  });

  it('unicode string should be ok', () => {
    const svg = toSVG("Xuân Nghĩa")
    fs.writeFileSync(__dirname + '/output/test-2.svg', svg)
    expect(svg).toBeDefined()
  });

  it('a long string should be ok', () => {
    const svg = toSVG("lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt ut labore et dolore magna aliqua")
    fs.writeFileSync(__dirname + '/output/test-3.svg', svg)
    expect(svg).toBeDefined()
  });


  it('revert color', () => {
    const svg = toSVG("lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt ut labore et dolore magna aliqua", {
      foregroundColor: '#ffffff',
      backgroundColor: '#000000'
    })
    fs.writeFileSync(__dirname + '/output/test-4.svg', svg)
    expect(svg).toBeDefined()
  });

  it('with logo', () => {
    const svg = toSVG("lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt ut labore et dolore magna aliqua", {
      logo: {
        url: logoUrl,
      }
    })
    fs.writeFileSync(__dirname + '/output/test-5.svg', svg)
    expect(svg).toBeDefined()
  })


  it('with logo custom size', () => {
    const svg = toSVG("lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt ut labore et dolore magna aliqua", {
      logo: {
        url: logoUrl,
        size: 80
      },
    })
    fs.writeFileSync(__dirname + '/output/test-6.svg', svg)
    expect(svg).toBeDefined()
  })

  it('no padding', () => {
    const svg = toSVG("lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt ut labore et dolore magna aliqua", {
      padding: 0
    })
    fs.writeFileSync(__dirname + '/output/test-7.svg', svg)
    expect(svg).toBeDefined()
  })
});